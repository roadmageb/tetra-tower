...
public class WpBWSpear : Weapon
{
    ...
    public override void PlaySkill(int skillNum, int option)
    {
        ...
        {
            case 0:
                EffectPool.Instance.StartEffect(GameManager.Instance.effectAnim["BWSpear00"], trns.position + Vector3.Scale(new Vector3(2, 1, 0), trns.localScale), trns.localScale);
                ...
                break;
            case 1:
                EffectPool.Instance.StartEffect(GameManager.Instance.effectAnim["BWSpear01"], trns.position + Vector3.Scale(new Vector3(2, 1, 0), trns.localScale), trns.localScale);
                if (PlayerController.Instance.controller.m_Grounded)
                {
                    PlayerController.Instance.GetComponent<Rigidbody2D>().velocity = new Vector2(0, 0);
                }
